  ...
  return (
    <div className="flex flex-col md:flex-row mt-10 gap-8 mx-4 md:mx-20">
      <div className="w-full md:w-1/2 flex gap-4">
        <div className="flex flex-col gap-2">
          {image.map((each, index) => (
            <img onClick={()=> setCurrImage(each)} key={index} src={each} alt={name} className="w-20" />
          ))} 
        </div>
        <div className="mb-4 md:mb-0">
          <img src={currImage} alt={name} className="w-full md:w-[450px] mx-auto rounded-lg shadow-lg" />
        </div>
      </div>
      <div className="w-full md:w-1/2 flex flex-col gap-2">
            ...
            {sizes.map((item, index) => (
              <button
                ...
                className={`border-2 border-emerald-600 p-3 w-16 m-1 text-center text-sm font-semibold rounded-lg 
                  ${size === item ? "border-orange-500 text-orange-500" : "hover:bg-emerald-600 hover:text-white"}`}
        ...
        <div className="mt-6">
          <button className="bg-black text-white w-full md:w-auto py-2 px-6 rounded-md hover:bg-gray-800 transition duration-300" onClick={addToCart}>
            ADD TO CART
          </button>
        </div>
